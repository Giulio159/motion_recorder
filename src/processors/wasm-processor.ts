...
type WasmExports = {
  ...
  process_frame: (input: number, output: number, width: number, height: number, threshold: number, decay: number, gain: number) => void;
};
...
export class WasmFrameProcessor implements FrameProcessor {
  ...
  static async create(url = '/wasm/motion_processor.wasm'): Promise<WasmFrameProcessor> {
    const response = await fetch(url);
    if (!response.ok) throw new Error(`WASM non disponibile: ${response.status}`);
    const result = await WebAssembly.instantiateStreaming(response, {});
    return new WasmFrameProcessor(result.instance.exports as unknown as WasmExports);
  }

  resize(width: number, height: number): void {
    const bytes = width * height * 4;
    if (bytes === this.byteLength) return;
    if (this.inputPtr) this.wasm.release(this.inputPtr);
    if (this.outputPtr) this.wasm.release(this.outputPtr);
    this.width = width;
    this.height = height;
  ...
  reset(): void { this.wasm.reset_processor(); }
